...
## 응답 언어 (필수 준수)
- **모든 대답은 한국어로 작성할 것** — 사용자 설명, 작업 보고, 에러 설명, 요약, 결론 모두 한국어 사용
- 코드/명령어/파일 경로/식별자는 원문 유지, 자연어 텍스트만 한국어로 출력
- 도구·라이브러리 이름 등 고유명사는 원어 표기 가능 (예: Vita3K, CRILAYLA)
...
- **리서치(Research)**: 생소한 포맷/프로토콜 역공학(CPK·CRILAYLA·FTX 등), Vita3K 동작 원리 조사, 외부 패치/역공학 사례 찾기
- **리뷰(Review)**: 완료된 패치/코드의 품질 검토, 범위 밖 부작용 확인, 대안 비교
- **막힌 디버깅**: 2시간 이상 혹은 2개 이상의 가설이 연달아 실패한 상황
- **OPENING NARRATION MBS 같은 미해결 이슈**: 두 CLI의 독립 의견을 반드시 비교

### 적용 제외 (호출하지 말 것)
- 단순 파일 수정, 이미 결정된 작업의 기계적 실행
- 테스트 실행, 빌드, 커밋 같은 반복 작업
- 명백한 버그(오타·누락된 import 등)의 즉시 수정
- 사용자가 "그냥 해줘"처럼 협의 생략을 명시한 경우
...
### 절차
1. **컨텍스트 요약**: 현재 상황·시도한 것·남은 옵션을 3~5줄로 요약
2. **동일 프롬프트로 둘 다 호출** (병렬로 Bash 두 번 실행 — 독립성 보존)
3. **결과 비교**:
...
- **응답 시간이 길 수 있음** — Bash 호출 시 `run_in_background: true` + `timeout 600000` 권장
- **수렴 결과도 맹신 금지** — 둘 다 같은 착각을 할 수 있으므로, 실제 코드/데이터로 교차검증
- 호출 실패(네트워크·인증) 시 사용자에게 즉시 보고하고 단독 진행 여부 확인
