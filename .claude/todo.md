...
## 진행 중 (2026-05-25 사용자 보고 2종)

### #1 대사 '['/']' 깨짐 — 뜯/뜸 RUNTIME_OVERLAY 충돌 재발 (수정 중)
- 증상: "물어뜯어"→"물어[어", "손님이 뜸해서"→"손님이 ]해서" (스크린샷 2장)
- 원인: 뜯 0x8B6F(河 local 283='[')·뜸 0x8B71(河 local 285=']'). 2350자 확장 때
  자연 완성형 위치로 되돌아가 RUNTIME_OVERLAY('['/']')와 재충돌. (과거 0x8EE9/0x8EEA로
  relocate했던 것이 회귀)
- 수정: 뜯→0x8EE9(local 968), 뜸→0x8EEA(local 969) relocate(양방향). 폰트 재생성+빌드.
- 명백한 회귀 버그라 codex/gemini 협의 생략(과거 딱/량/럴/랴/봐 relocate 연장).

### #2 오의명 vs 오의 텍스처 불일치 일괄 검수 (codex+gemini 협의)
- 사용자 보고: 아이템 오의명과 텍스처(E8E01EAF 등)의 표기가 다른 곳이 아직 많음
- 전수조사 → codex+gemini 일괄 검수 진행 예정
