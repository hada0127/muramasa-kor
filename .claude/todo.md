...
> 현재 릴리스: **v1.1.1** 🎉 (2026-05-27 — 튜토리얼 '멋지다!' 느낌표→한자(取) 깨짐 수정, 이슈 #14 해결)
> 텍스처 편집: `python tools/ui_editor/server.py` (http://127.0.0.1:8765)

---

## [2026-05-27] GitHub 이슈 #12 ○/✕ 선택 버튼 — ✕버튼 추가팩 ✅ 구현 완료(미발행)

- 생성기/레지스트리/✕텍스처/추가팩빌드/독립패처/UI에디터패널/문서 전부 완료·커밋.
- ✕판 텍스처 2개 픽셀 검증 OK(변경이 확인셀·○링슬롯에만 국한). 추가팩 zip dry-run OK.
- **남은 것(사용자 확인 필요)**: 버전 표기(현 1.1.1 유지 vs 1.2.0 bump) + `gh release` 발행.
  본편 패치 자체는 무변경이라 ✕ 추가팩만 기존/새 릴리스에 자산으로 첨부하면 됨.
- 상세는 success.md 참조.
