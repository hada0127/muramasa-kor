...
### 근본 원인
- `jp_messages.json._itemdata`는 **JP NMS(3565)** 인덱스 기준인데 게임은 **US NMS(1177)** 로드.
  JP에만 있는 더미(`BOSS鬼助/百姫 N本目` 12슬롯 506~517, `回復X1~4` 8슬롯 578~585)가 US엔 없어
  US 액세서리 슬롯이 시프트(+12 → +20).
- `build_patch.py us_itemdata_hybrid`가 `[0,594)`를 JP idx i→US idx i **1:1** 매핑 → US564
  (Bracelet of the Deva King/인왕의 팔찌)에 jp[564]="쌍나루코" 주입. [506,564] 전 액세서리 +12
  어긋남 + BOSS 더미 6개가 메뉴에 유령 노출.

### codex + agy 협의
- **agy**: offset 규칙(+0/+12/+20) — 코드에 직접 적용까지 수행. **codex**: 수동 대응표(B) 추천하되
  핵심은 "조용히 틀리는 버그라 빌드 시 앵커/드리프트 검증 필수". → **결합 채택**: offset 규칙 +
  앵커 드리프트 가드(US506=鎖帷子/552=二繋ぎの鳴子/564=仁王の腕輪/566=琥珀の勾玉 불일치 시 빌드 중단).

### 수정·검증
- `tools/build_patch.py`: patch_patch `us_itemdata_hybrid` offset 보정 + 앵커 가드. patch_main
  `_itemdata_US`도 [506,566) +12 보정(base는 DLC 곡옥 없어 곡옥/skill은 현행 유지, 미사용이나 일관성).
- 빌드 후 양쪽 US564="인왕의 팔찌", 보스 더미 잔존 0, **44개 액세서리 전수 영문↔한글 대조 정상**.
- 한글 아이템명은 일본어 원판 기준이라 영문 공략명과 다름(쌍나루코=Cowbell, 나루코=Triangle) = 정책.
- BOSS 더미 사실확인: `BOSS鬼助/百姫 N本目`=적 전용 내부데이터(효과 敵用 所持/使用するな, 영문판도
  제거). "영문 oss 표시"=사실아님. 공략 "2회차 보스아이템"=별개의 정상 드롭 액세서리. 더미 숨김이 정답.
- **✅ v1.3.1 발행** (gh release --latest, 5자산). 최종 배포 CPK 내부 US564=인왕의 팔찌 검증.
  push 228a069, tag v1.3.1. 인게임 최종 확인은 macOS 한계로 제보자/사용자.
...
- **#16-2(텍스처)**: HUD 퀵슬롯 `E8E01EAF5D41DB52` K10 원본 "Sake"→"술"을 **"반야탕"**으로 통일(메뉴 般若湯과 일치). 인게임 확정.
- **#16-1(텍스처 2개 정렬)**: 백사전 선택화면은 **A3BE57CE(장 이름)·88A7A102(에피소드 제목) 두 텍스처의 백사전을 겹쳐** 그림.
  白蛇만 두 텍스트가 똑같이 "백사전"이라 겹쳐야 하는데 어긋나 이중. (확정 실험: A3BE→우측/88A7→하단 이동 후 캡처해 둘 다 렌더됨 입증.)
  - **근본원인**: UI에디터 도입(05-22) 직후 "1박스=1단어 중앙정렬" 리워크가 A3BE 백사전을 박스중앙(cx640)+폰트변경으로 옮겨 어긋남. (UI에디터 前 cx582 붓글씨라 88A7과 정렬됐었음 — 사용자가 정확히 지적.)
  - **조치(사용자 지시 = 그리운 경찰감성체로 통일)**: A3BE config K8 `w`1281→1164(cx640→582)·clear박스 위로 확장(cy3401)으로
    pre-UI-editor 위치 복원. 88A7(수동 넉아웃)의 붓글씨 백사전을 A3BE Griun 글리프(폭274 비율유지)로 교체(蛇 한자꼬리·타3편 자막 보존).
    인게임 반복 확인하며 88A7 +7px 우측 미세정렬로 마무리.
  - **도구**: texture_localize에 `glyph_image` region(폰트 대신 PNG 글리프 합성) 기능 추가(붓글씨 옵션 보존, `textures/sprites/baeksajeon_brush.png`).
- **검증**: macOS Vita3K 인게임 8회 반복 캡처(사용자 협업) — 이중 제거·단일 백사전·Griun 통일 확인. zip 내 텍스처 해시=repo 일치.
- **✅ v1.2.2 발행·클로즈**: build_release로 패처 zip + ✕추가팩 빌드. tag v1.2.2 + gh release(--latest) + 이슈 #16 댓글·클로즈.
