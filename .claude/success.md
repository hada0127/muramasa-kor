...
### 미해결 (별개 이슈)
- **#4 럴럴**: sysmsg#225 `이 ##(무기)는 만들 수 없습니다`의 `##` placeholder가 무기명으로 치환 안 되고 literal 출력 → '#'(0x23)이 scheme-B 셀 963을 읽는데 거기에 (Form:%s 수정 때 옮긴) 한글 **럴**이 있어 `럴럴`로 보임. v0.9.0에도 동일 존재 → 매핑 복원으로 안 고쳐짐. 별도 수정 필요.
