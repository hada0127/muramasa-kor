...
- box: `武蔵` → `무사시`
- banner: `江戸 網釜藩邸 上屋敷` → `에도 아미가마 번저 가미야시키`
- banner: `証城寺跡` → `쇼조지 옛터`
- character: `綱釜千代子` → `쓰나가마 치요코`
...
- banner: `綱谷藩勝尾城` → `쓰나야 번 가쓰오성`
- banner: `綱谷藩勝尾城回廊` → `쓰나야 번 가쓰오성 회랑`
- banner: `綱谷藩葛銀字庭` → `쓰나야 번 갈은자 정원`
- banner: `由比ヶ浜` → `유이가하마`
...
- banner: `金剛山山頂高天原入口` → `곤고산 산정 다카마가하라 입구`
- character: `犬神徳川綱吉` → `이누가미 도쿠가와 쓰나요시`
...
- box: `美濃` → `미노`
- box: `武蔵` → `무사시`
- banner: `鳴神城広間の場` → `나루카미성 넓은 방의 장`
- banner: `江戸城天守` → `에도성 천수각`
- character: `犬神徳川綱吉` → `이누가미 도쿠가와 쓰나요시`
