...
def write_release_notes(notes_path: Path, version: str, zip_name: str) -> None:
    notes = (
        ...
        f"\n"
        f"주요 변경 사항:\n"
        f"- v0.9.1 — 받침 표시·인명·문장 오타 모음 패치:\n"
        f"  - 자주 깨지던 받침 38자(똥/땐/뺏/쩌/썰/쥐/땡/엽/섣/뤄/믐/픔/촌/윈/믄/캇/랭/돕/겪/턱/꾀/쩨/닮/뭣/줬/짤/삘/꿋/굳/켕/홍/찜/챵/녁/렛/궈/꿨/썼) 표시 보정\n"
        f"    → '빠빠이'가 '빡빡이'로, '짜짜한'이 '쩨쩨한'으로, '땡중' 표기 정상화 (#7, #8)\n"
        f"  - '뜯/뜸' 충돌 자리 이동 → '이로 물어[ 어 조각내주마'가 '이로 물어뜯어 조각내주마'로 정상 표시 (#7)\n"
        f"  - 무기 대장간의 '럴럴' 표기가 정상 단어로 표시되도록 폰트 슬롯 정리 (#4)\n"
        f"  - 키스케편 진쿠로 대사 '마대로' → '맘대로' (받침 '맘' 추가, #5)\n"
        f"  - 油田 인명 표기 통일: '유다' → '아부라다' (3건, #8)\n"
        f"  - 어색한 문장 다듬기: '이 불시말을 어찌 수습할 셈이냐' → '이 사태를 어찌 수습할 셈이냐',\n"
        f"    '이런 곳을 노리면 버틸 수 없어' → '이런 상태에 있다가는 버틸 수 없어'\n"
        f"  - 해골 무리 직위명 통일: '상닌/하닌' → '상급닌자/하급닌자' (대사·시스템 9건),\n"
        f"    줄바꿈 박스 한도(27자) 맞춰 관련 4개 대사 재정리\n"
        f"  - 저장/덮어쓰기 다이얼로그 다이아 박스 '네/아니오' 위의 영문 'Yes/No' 잔상 제거\n"
        f"    (텍스처 8725F040AEE76DFC 보정)\n"
        f"  - 특정 화면에서 한글 메시지가 일본어 한자로 뜨던 문제 수정\n"
        f"    (게임이 추가로 사용하는 폰트 텍스처 8665CE08에도 한글 글리프 import 적용)\n"
        f"  - 알려진 미해결: 받침 누락 잔여 54자(괄/굶/귈/꿇/끕 등)는 폰트 슬롯 한계로 다음 작업 사이클로 이월,\n"
