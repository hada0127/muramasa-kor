...
def write_release_notes(notes_path: Path, version: str, zip_name: str) -> None:
    notes = (
        ...
        f"2. 기존 한글 패치 적용본이 있다면, 처음 설치 직후의 원본 CPK로 복원하거나\n"
        f"   본편/업데이트를 다시 설치한 뒤 진행합니다. 패치된 CPK 위에 다시 덮어씌우면\n"
        f"   원본 해시가 맞지 않아 패처가 중단됩니다.\n"
        f"3. `{zip_name}`을 원하는 위치에 압축 해제합니다.\n"
        f"4. Windows: `apply_windows.bat`을 실행합니다.\n"
        f"5. macOS/Linux: `python3 apply_patch.py`를 실행합니다. macOS에서는\n"
        f"   `apply_macos.command`를 실행해도 됩니다.\n"
        f"6. 패처는 원본 CPK 해시 확인, 원본 백업 생성, CPK 바이너리 패치 적용,\n"
