...
def write_release_notes(notes_path: Path, version: str, zip_name: str) -> None:
    notes = (
        ...
        f"    (`무너졌다. 유일한`, `보였건만.`, `법인가.` 등)\n"
        f"  - 마침표와 말줄임표 뒤 같은 줄 후속 문자가 있을 때 공백 1칸 유지\n"
        f"  - 본편+DLC 대사를 29 전각 폭 기준 greedy fill로 재정리하고 fixed-point 수렴\n"
        f"  - 결과: 한국어 대사 4,170줄 → 4,009줄, 3줄 메시지 235건 → 125건,\n"
        f"    KO p95=29.0 / max=29.0\n"
        f"  - DLC 엔딩의 미한글화 `完` 텍스처를 본편 한글 `완` 텍스처와 맞춰 보정\n"
        f"  - 추가 export 텍스처 감사 결과를 문서화하고 신규 지명 텍스처\n"
        f"    `峠の茶屋` → `고갯마루 찻집` 추가\n"
        ...
        f"  - 지명 텍스처와 시스템 메시지 표기 통일\n"
        f"  - 한글 폰트와 UI 텍스처 import 개선\n"
        f"\n"
        f"설치 방법:\n"
