...
def write_release_notes(notes_path: Path, version: str, zip_name: str) -> None:
    notes = (
        ...
        f"\n"
        f"주요 변경 사항 (이슈 #2, #3 해결):\n"
        f"- 화폐 단위 통일 (이슈 #3): 가마꾼 대사·상점 시스템 메시지의 '돈'을 '문'으로 정리.\n"
        f"  - sysmsg '文を支払い' → '문을 지불' (이전: '돈을 지불')\n"
        f"  - _itemdata 효과 백분율 '푼'(7건) → '%'로 통일, 조사 자연화\n"
        f"  - 큰 단위 '냥' + 작은 단위 '문' + 백분율 '%' (현 텍스처 100% 호환)\n"
        f"- DLC 1 바케네코 전투 결과창 '시간' 표시 깨짐 수정 (이슈 #2 항목 3):\n"
        f"  - 게임이 시간을 hh:mm:ss로 출력 시 전각 콜론(SJIS 0x8146)이 한글 '봐'로\n"
        f"    렌더되던 문제. '봐' SJIS를 0x8EEF로 재배치 + cell 454에 콜론 글리프\n"
        f"    오버레이로 해결. NMS '봐' 73회 모두 새 위치에서 정상 표시.\n"
        f"- DLC 1 옵션 메뉴 '연습' 용어 통일 (이슈 #2 항목 4):\n"
        f"  - DF66CADDABE022E3.kra atlas layer14의 '연 습' → '단 련'으로 수정.\n"
        f"    게임 내 설명(sysmsg #486-488 鍛錬)과 라벨 용어 일치.\n"
        f"- DLC 1 장비란 'Form:%s' hardcoded 영문 깨짐 수정 (이슈 #2 항목 5):\n"
        f"  - 게임이 'Form:Okoi/Miike/Avatar' 영문을 eboot에서 hardcoded로 출력하던 것이\n"
        f"    한글 폰트 매핑으로 '떡량렬랏'처럼 깨짐. 영문판 검증으로 원문 확정 후\n"
        f"    딱/량/럴/랴 SJIS 재배치(0x8EE2-0x8EE5) + cell 262/303/306/301에 F/o/r/m\n"
        f"    영문 글리프 오버레이. 콜론 위치(x=8)도 조정해 다음 한글과 겹치지 않게 함.\n"
        f"  - 결과: 'Form:오코이' / 'Form:미케' / 'Form:화신' 정상 표시.\n"
        f"- vita3k_mode.py 신규: english/korean/status 모드 전환 유틸\n"
        f"  (재패치 시 영문 원문 확인용. 한글 자산 안전 백업).\n"
        f"\n"
        f"설치 방법:\n"
