...
def write_release_notes(notes_path: Path, version: str, zip_name: str) -> None:
    notes = (
        ...
        f"\n"
        f"주요 변경 사항:\n"
        f"- v0.9.3 — 결과 화면 '평가' 텍스처 클리핑 수정:\n"
        f"  - 전투 결과 화면의 '평가'(Rating) 라벨이 게임에서 잘려 보이던 문제 수정.\n"
        f"    텍스처(DF66CADD) 내 글자 영역이 과대 설정돼 화면 UV 밖으로 넘쳐 잘리던 것을\n"
        f"    영역 축소·재배치로 정상화\n"
...
GUI_BUNDLE_README = """\
==================================================
  무라마사 리버스 한글 패치 — 딸깍 설치 도구
==================================================

마우스 클릭만으로 한글 패치를 만드는 도구입니다.
게임 원본은 들어있지 않습니다 — 본인이 가진 원본 게임으로 패치를 만듭니다.
...
────────────────────────────────────────────────
  ...
  ▶ Windows   : MuramasaPatcher.exe  더블클릭
                (파이썬 설치 필요 없음)
                · 파란 SmartScreen 경고 → "추가 정보" → "실행"

  ▶ Mac       : 실행_Mac.command  더블클릭  (파이썬 3.9+ 필요)
                · "확인되지 않은 개발자" 경고 → Finder 우클릭 → 열기

  ▶ Linux     : sh 실행_Linux.sh
...
────────────────────────────────────────────────
  ...
  ■ 에뮬레이터(Vita3K)로 즐긴다   ← 대부분 여기!
     ─────────────────────────────────────
      ...
      ※ 되돌리려면? "원본으로 복원" 체크 후 [패치 시작].


  ■ 실기(진짜 PS Vita 본체)로 즐긴다   [베타]
     ─────────────────────────────────────
...
────────────────────────────────────────────────
  ...
  PC 성능에 따라 1~5분 걸립니다.
  로그가 끝나고 "완료" 창이 뜨면 성공입니다. 게임을 켜보세요. 🎉
...
────────────────────────────────────────────────
  ...
  도구에서 결정 버튼을 ✕ 로 바꿨다면,
   · Vita3K  : 설정 → Enter Button Assignment = Cross
   · 실기    : Vita 설정 → 시스템 → 결정 버튼 = ✕(Cross)
  기본 ○ 그대로 두었다면 신경 쓸 것 없습니다.
...
────────────────────────────────────────────────
  ...
  · 한글이 전혀 안 나옴
      에뮬: 위 "Import Textures" 켜고 재시작.
      실기: rePatch가 켜져 있는지 + ux0:/rePatch/ 에 복사됐는지 확인.
  · "패치 시작"에서 오류
      원본 CPK 경로가 맞는지, 1.06 업데이트가 깔린 원본인지 확인.
  · 일부 화면만 영문/한자
      DLC 화면이면 도구에서 DLC Pack 경로도 넣었는지 확인.

  실기 표시는 아직 베타입니다. 문제가 생기면 어떤 화면인지와 함께
  제보해 주시면 큰 도움이 됩니다.
"""
