...
def write_release_notes(notes_path: Path, version: str, zip_name: str) -> None:
    notes = (
        ...
        f"주요 변경 사항:\n"
        f"- v0.8.2 핫픽스:\n"
        f"  - DLC 찻집 메뉴 삼색경단 가격 `10문`의 `1` 숫자 sprite 복구\n"
        f"  - 메뉴용 A8E6 폰트의 흰색 stroke 제거. 메뉴 한글은 외곽선 없는 출력 유지\n"
        f"  - 네코마타편 엔딩 컷씬 멈춤 원인인 JP voice/SE placeholder의 US scemsg 주입 방지\n"
        f"  - 해당 placeholder는 원본 cue로 보존하고 `jp_index` 매핑에서 제외\n"
        f"- 본편+DLC 대사 줄 재정리 (8회 패스 누적, fixed-point 수렴):\n"
        f"  - greedy first-line fill 알고리즘 도입(`tools/condense_dialogs.py`)\n"
        f"  - 박스 폭 측정(결정 우측 = 27 전각 단위) 기반 한도 max_width=27 적용\n"
        f"  - 단어 경계 split, placeholder/화자 라벨 보존, 의미 변경 없음\n"
        f"  - 결과: 3줄 메시지 1,273 → 229 (-82%), 4줄+ 0건. 좌측 치우침 해소\n"
        f"- 문장 부호 보정·정렬 (`tools/fix_punctuation.py`):\n"
        f"  - JP 원문 종결 부호(。！？…) 기준 한국어 종결어미 마침표 누락 130건 보정\n"
        f"  - `...`·`.` 뒤 같은 줄 후속 문자 있을 시 공백 1칸 자동 삽입 (가독성)\n"
        f"  - codex+gemini 협의로 자동 룰 결정, 의문 케이스는 별도 리포트로 격리\n"
        f"- 리디바탕 폰트 검은 외곽선 추가 (이름/시스템 메시지 가독성 개선):\n"
        f"  - 원본 일본어 폰트 텍스처에 있던 stroke가 한글 import에서 누락되어\n"
        f"    밝은 배경에서 캐릭터 이름이 안 보이던 문제 해결\n"
        f"  - `tools/auto_font_import.py`/`hd_font_import.py`에 stroke_width=2 +\n"
        f"    본체 22→20 / 18→16 (셀 침범 방지). HD 베이스는 stroke=2*scale로 비례\n"
        f"- 지명 텍스처 ↔ 시스템 메시지 표기 통일:\n"
        f"  - 텍스처 7건 정정 (武蔵 '스푸/센슈' → '무사시', 駿河 '교조구' → '스루가',\n"
        f"    遠江 '몬젠' → '도토미', 大和 '요시노' → '야마토', 美濃 '오미슈' → '미노' 등)\n"
        f"  - 메시지 38건 표기 통일·오역 교정 (秋葉山 '치바산' → '아키바산',\n"
        f"    駿府 '사가미' → '슨푸', 鏡見家 '카미가' → '카가미가' 등)\n"
        f"  - `place_name_mapping.json` 권위 데이터 '토토미' → '도토미' 등 정정\n"
        f"- macOS/Linux/Windows 경로 platform 분기 (auto_font/hd_font import):\n"
        f"  - `VITA3K_ROOT`/`VITA3K_EXPORT_DIR`/`VITA3K_IMPORT_DIR`/`HD_PACK_DIR`\n"
        f"    환경변수 오버라이드 지원. 다중 OS 작업 흐름 정식 지원\n"
        f"\n"
        f"설치 방법:\n"
